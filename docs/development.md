...
- The monorepo resolves types through compiled declaration contracts, not source. Package `exports` `types` conditions point at `build/`, and declaration emit is a types-only artifact. See [docs/type-system.md](type-system.md) for the model and its remaining tasks.
