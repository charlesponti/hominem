...
- The monorepo resolves types through compiled declaration contracts, not source. Package `exports` `types` conditions point at `build/`, and declaration emit is a types-only artifact. The declaration watcher runs automatically as a persistent turbo task under `pnpm dev` (and `dev:api`/`dev:career`/`dev:finance`) — composite packages via `tsc -b` plus the api/rpc emits rebuild on change, so editors and terminal checks see fresh types within seconds. `just types watch` runs the same watcher standalone, and `mobile:dev` (Expo) doesn't include it — pair with `pnpm dev:types` when running omiro only. Runtime (tsx, metro, vite) runs from source and is unaffected. If a type change ripples further than one hop, restart the TypeScript server. See [docs/type-system.md](type-system.md) for the model and its remaining tasks, and [docs/type-performance.md](type-performance.md) for why the watcher is shaped the way it is and what else was tried to speed up type-checking.
