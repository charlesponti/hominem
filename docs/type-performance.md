...
**`packages/rpc/tsconfig.json` was not.** It has had `references: [{ path: "../chat" }]` since 2026-08-25 (`git blame`; added directly to `main`, predates any of this investigation) — while `AGENTS.md`'s text, at the time this was found, claimed `packages/rpc/tsconfig.json` "already" followed the zero-references pattern. It doesn't, and apparently hasn't for a while, without incident. This is a real, live discrepancy between the documented invariant and actual repo state, not a hypothetical — flagged in `AGENTS.md` and here rather than silently resolved either direction (leaving `packages/rpc` as an unreconciled exception, not a second sanctioned pattern to copy for a new package).

**Practical rule:** if you're adding a `typeof app`-style inference boundary, give it zero `references` entries, full stop — resolve every dependency via plain paths to `build/*.d.ts`, same as `services/api`. Don't point to `packages/rpc`'s tsconfig as precedent.
