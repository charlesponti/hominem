...
export async function personalFinanceRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/location-comparison', { preHandler: verifyAuth }, async (request, reply) => {
    try {
      ...
      const validated = locationComparisonSchema.parse(request.body)
      ...
      const adjustedTotalExpenses = adjustedExpenses.reduce(
        (sum, expense) => sum + expense.adjustedAmount, 0
      )
      ...
      const qolChangePercent = ((adjustedSurplus / adjustedIncome) - 
                               (currentSurplus / validated.income)) * 100
  ...
  fastify.post('/travel-cost', { preHandler: verifyAuth }, async (request, reply) => {
    try {
      ...
      const costs = destinationCosts[validated.destination] || { 
        flight: 500, hotel: 200, food: 65, activities: 50 
      }
