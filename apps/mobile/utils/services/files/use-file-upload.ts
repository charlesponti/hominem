...
export type { MobileUploadAsset, MobileUploadState, MobileUploadBatchResult }
...
export async function performMobileUploads(
  ...
  for (const [index, asset] of assets.entries()) {
    ...
    try {
      ...
      const { fileId, key, uploadUrl, headers } = await api.getUploadUrl({
        originalName,
        mimetype,
        size: fileBlob.size,
      })
...
export function useFileUpload() {
  ...
  const uploadAssets = useCallback(
    async (assets: MobileUploadAsset[]): Promise<MobileUploadedAsset[]> => {
      ...
      const result = await performMobileUploads(apiClient.files as Parameters<typeof performMobileUploads>[0], assets, {
        onProgress: (progress) => {
