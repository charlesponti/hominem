...
export function useEmailOtpAuthRoute(input: {
  ...
  const searchParams = useMemo(() => new URLSearchParams(input.search), [input.search]);
  ...
  const [step, setStep] = useState<'email' | 'otp'>(
    initialEmail && searchParams.get('step') === 'otp' ? 'otp' : 'email',
  );
  const postAuthRedirect = useMemo(
    () =>
      resolveAuthRedirect(
        ...
        input.allowedRedirectPrefixes,
      ).safeRedirect,
    [input.allowedRedirectPrefixes, input.defaultRedirect, searchParams],
  );
