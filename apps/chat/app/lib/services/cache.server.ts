...
// Define specific types for different cache data
...
const caches = {
  transcription: new LRUCache<string, TranscriptionData>(CACHE_CONFIGS.transcription),
  fileProcessing: new LRUCache<string, FileProcessingData>(CACHE_CONFIGS.fileProcessing),
  tts: new LRUCache<string, TTSData>(CACHE_CONFIGS.tts),
  search: new LRUCache<string, SearchData>(CACHE_CONFIGS.search),
  chatResponse: new LRUCache<string, ChatResponseData>(CACHE_CONFIGS.chatResponse),
}

export type CacheType = keyof typeof caches

// Generate cache key from inputs
export function generateCacheKey(type: CacheType, ...inputs: unknown[]): string {
...
export class CacheService {
  ...
  static set<T>(type: CacheType, key: string, value: T): void {
    caches[type].set(key, value as never)
  }
  ...
  static getStats(type: CacheType): CacheStats {
  ...
  static getAllStats(): Record<CacheType, CacheStats> {
    return Object.keys(caches).reduce(
      (acc, type) => {
        acc[type as CacheType] = CacheService.getStats(type as CacheType)
        return acc
      },
...
export class TranscriptionCache {
  static async getOrCreate<T>(audioHash: string, creator: () => Promise<T>): Promise<T> {
...
export class CacheManager {
  ...
  static emergencyCleanup(): void {
    for (const type of Object.keys(caches)) {
      ...
      const toClear = Math.floor(keys.length / 2)
