...
function checkCommand(name: string, command: string, args: string[], required: boolean): CheckResult {
...
function checkPinnedVersion(name: string, runtimeVersion: string, pinnedVersion: string | null, required: boolean): CheckResult {
  if (!pinnedVersion) {
    return {
      name,
      required,
...
function runDoctor(): number {
  ...
  checks.push(checkPinnedVersion('Pinned Bun version', bunVersion ?? 'unknown', pinnedVersions.bun, true))
  checks.push(checkPinnedVersion('Pinned Node.js version', nodeVersion ?? 'unknown', pinnedVersions.node, true))
