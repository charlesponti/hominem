...
function generateTrace(targetPath: string = '.') {
  ...
  console.log('🔍 Generating TypeScript trace data...');
  ...
  try {
    ...
    if (existsSync(join(tracePath, 'trace.json'))) {
      console.log('⚠️  Warning: tsc exited with errors, but trace was generated\n');
      return tracePath;
    }

    console.error('\n❌ Failed to generate trace.');
...
function analyzeTrace(tracePath: string) {
  ...
  try {
    ...
    console.error('❌ Failed to parse trace file. The trace may be corrupted.');
  ...
  console.log('⏱️  Top 20 Slowest Type Checks:\n');
