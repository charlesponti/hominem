...
database_name="$(sanitize_database_name "${template_name}_verify_$(date +%s)_$$_${RANDOM}")"
...
DATABASE_URL="$database_url" "$ROOT_DIR/scripts/run-goose.sh" up >/dev/null
DATABASE_URL="$database_url" "$ROOT_DIR/scripts/run-goose.sh" up >/dev/null

status="$(DATABASE_URL="$database_url" "$ROOT_DIR/scripts/run-goose.sh" status)"
printf '%s\n' "$status"
