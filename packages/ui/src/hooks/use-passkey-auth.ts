...
function normalizeRequestOptions(raw: RawPublicKeyCredentialRequestOptions): PublicKeyCredentialRequestOptions {
...
function normalizeCreationOptions(raw: RawPublicKeyCredentialCreationOptions): PublicKeyCredentialCreationOptions {
...
export function usePasskeyAuth(options: UsePasskeyAuthOptions = {}) {
  ...
  const authenticate = useCallback(async (): Promise<PasskeyAuthResult | null> => {
    ...
    try {
      ...
      const rawPayload = (await optionsResponse.json()) as { options?: RawPublicKeyCredentialRequestOptions } | RawPublicKeyCredentialRequestOptions
      // The API may return options directly or wrapped in an `options` key
      const rawOptions = (rawPayload as { options?: RawPublicKeyCredentialRequestOptions }).options ?? (rawPayload as RawPublicKeyCredentialRequestOptions)
  ...
  const register = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      const rawRegPayload = (await optionsResponse.json()) as { options?: RawPublicKeyCredentialCreationOptions } | RawPublicKeyCredentialCreationOptions
      // The API may return options directly or wrapped in an `options` key
      const rawRegOptions = (rawRegPayload as { options?: RawPublicKeyCredentialCreationOptions }).options ?? (rawRegPayload as RawPublicKeyCredentialCreationOptions)
      // Normalize base64url strings to ArrayBuffers for the WebAuthn API
