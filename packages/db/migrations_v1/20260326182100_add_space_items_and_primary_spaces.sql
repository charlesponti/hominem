  ...
  SELECT EXISTS (
    ...
    WHERE entity.entity_table = target_entity_table
      ...
      AND (
        ...
        OR entity.owner_user_id = auth.current_user_id()
        OR (
...
CREATE TRIGGER app_chats_sync_entity_registry
  ...
  FOR EACH ROW
  EXECUTE FUNCTION app.sync_entity_registry('owner_user_id', 'primary_space_id');
...
INSERT INTO app.space_items (space_id, entity_table, entity_id, added_by_user_id, added_at)
SELECT primary_space_id, 'app.tasks'::regclass, id, owner_user_id, created_at
FROM app.tasks
WHERE primary_space_id IS NOT NULL;

INSERT INTO app.space_items (space_id, entity_table, entity_id, added_by_user_id, added_at)
SELECT primary_space_id, 'app.chats'::regclass, id, owner_user_id, created_at
FROM app.chats
WHERE primary_space_id IS NOT NULL;
