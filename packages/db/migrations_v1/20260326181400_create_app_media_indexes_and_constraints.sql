...
CREATE UNIQUE INDEX app_music_artists_owner_source_external_key
  ON app.music_artists (owner_user_id, source, external_id)
  WHERE external_id IS NOT NULL;

CREATE INDEX app_music_artists_owner_name_idx
  ON app.music_artists (owner_user_id, lower(name));

CREATE UNIQUE INDEX app_music_albums_owner_source_external_key
  ON app.music_albums (owner_user_id, source, external_id)
  WHERE external_id IS NOT NULL;

CREATE INDEX app_music_albums_owner_title_idx
  ON app.music_albums (owner_user_id, lower(title));
...
CREATE UNIQUE INDEX app_music_tracks_owner_source_external_key
  ON app.music_tracks (owner_user_id, source, external_id)
  WHERE external_id IS NOT NULL;

CREATE INDEX app_music_tracks_owner_title_idx
  ON app.music_tracks (owner_user_id, lower(title));
...
CREATE UNIQUE INDEX app_music_playlists_owner_source_external_key
  ON app.music_playlists (owner_user_id, source, external_id)
  WHERE external_id IS NOT NULL;

CREATE INDEX app_music_playlists_owner_created_at_idx
  ON app.music_playlists (owner_user_id, created_at DESC);
...
CREATE INDEX app_music_listens_owner_started_at_idx
  ON app.music_listens (owner_user_id, started_at DESC);
...
CREATE INDEX app_video_views_owner_content_type_idx
  ON app.video_views (owner_user_id, content_type);
...
DROP INDEX IF EXISTS app_music_playlists_owner_created_at_idx;
