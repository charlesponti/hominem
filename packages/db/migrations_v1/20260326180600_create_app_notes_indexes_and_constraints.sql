...
ALTER TABLE app.chats
  ...
  ADD CONSTRAINT app_chats_last_message_after_created_check CHECK (last_message_at >= created_at);
...
CREATE INDEX app_note_versions_created_by_user_id_idx
  ON app.note_versions (created_by_user_id)
...
CREATE INDEX app_tag_assignments_assigned_by_user_id_idx
  ON app.tag_assignments (assigned_by_user_id)
...
CREATE INDEX app_chats_last_message_at_idx
  ON app.chats (owner_user_id, last_message_at DESC);
...
DROP INDEX IF EXISTS app_note_versions_created_by_user_id_idx;
DROP INDEX IF EXISTS app_tag_assignments_assigned_by_user_id_idx;
