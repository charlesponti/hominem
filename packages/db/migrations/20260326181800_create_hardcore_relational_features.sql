...
ALTER TABLE app.note_shares
  ADD COLUMN granted_by_user_id uuid REFERENCES auth.users(id) ON DELETE SET NULL,
  ADD COLUMN expires_at timestamptz,
  ADD COLUMN revoked_at timestamptz,
  ADD COLUMN access_period tstzrange GENERATED ALWAYS AS (
    tstzrange(created_at, coalesce(revoked_at, expires_at, 'infinity'::timestamptz), '[)')
  ) STORED;
...
CREATE INDEX app_note_shares_granted_by_user_id_idx
  ON app.note_shares (granted_by_user_id)
...
CREATE INDEX app_space_members_space_id_user_id_idx
  ON app.space_members (space_id, user_id);
...
ALTER TABLE app.space_invites
  ...
  ADD COLUMN invite_window tstzrange GENERATED ALWAYS AS (
    tstzrange(
      created_at,
      coalesce(accepted_at, revoked_at, expires_at, 'infinity'::timestamptz),
      '[)'
    )
  ) STORED;
...
CREATE TABLE app.task_assignments (
  ...
  CONSTRAINT app_task_assignments_membership_fkey
    FOREIGN KEY (space_id, assignee_user_id, PERIOD assignment_period)
    REFERENCES app.space_members(space_id, user_id, PERIOD membership_period),
...
CREATE INDEX app_task_assignments_assignee_user_id_idx
  ON app.task_assignments (assignee_user_id);
...
CREATE INDEX app_entity_links_owner_user_id_idx
  ON app.entity_links (owner_user_id, created_at DESC);
...
CREATE POLICY app_space_invites_select_policy ON app.space_invites
  ...
  USING (
    ...
    OR (
      invited_user_id = auth.current_user_id()
...
CREATE POLICY app_task_assignments_select_policy ON app.task_assignments
  ...
  USING (
    auth.is_service_role()
    OR assignee_user_id = auth.current_user_id()
    OR EXISTS (
      SELECT 1
      FROM app.tasks task
      WHERE task.id = task_assignments.task_id
        AND (
          task.owner_user_id = auth.current_user_id()
          OR (
...
CREATE POLICY app_task_assignments_owner_write_policy ON app.task_assignments
  ...
  USING (
    ...
    OR EXISTS (
      ...
      FROM app.tasks task
      WHERE task.id = task_assignments.task_id
        AND task.owner_user_id = auth.current_user_id()
    )
  )
  WITH CHECK (
    auth.is_service_role()
...
DROP INDEX IF EXISTS app.app_entity_links_owner_user_id_idx;
DROP TABLE IF EXISTS app.entity_links;

DROP INDEX IF EXISTS app.app_task_assignments_assignee_user_id_idx;
...
DROP INDEX IF EXISTS app.app_space_members_space_id_user_id_idx;
DROP INDEX IF EXISTS app.app_list_members_list_id_user_id_idx;
...
ALTER TABLE app.space_members
  ADD CONSTRAINT app_space_members_pkey PRIMARY KEY (space_id, user_id);

ALTER TABLE app.space_members
  DROP COLUMN IF EXISTS membership_period,
  DROP COLUMN IF EXISTS id;

DROP INDEX IF EXISTS app.app_note_shares_granted_by_user_id_idx;
