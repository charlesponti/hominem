...
describe.skipIf(!dbAvailable)('modern-finance budget integration', () => {
  ...
  const createUser = async (id: string): Promise<void> => {
    await db.execute(sql`
      ...
      values (${id}, ${`${id}@example.com`}, ${'Finance Budget User'})
  ...
  it('creates missing categories from transactions and returns analysis aggregates', async () => {
    ...
    const transactions = await queryTransactionsByContract({ userId: ownerId, limit: 50, offset: 0 })
    ...
    const afterDelete = await queryTransactionsByContract({ userId: ownerId, limit: 50, offset: 0 })
