...
async function queryAnalyticsTransactionsCore(
  ...
  let query = db
    ...
    .select([
      ...
      't.notes',
      't.merchant_name',
      sql<string>`coalesce((select min(tg_tag.name) from app.tag_assignments ti_tag join app.tags tg_tag on tg_tag.id = ti_tag.tag_id and tg_tag.owner_userid = ${parsed.userId} where ti_tag.entity_table = ${FINANCE_TRANSACTION_ENTITY_TYPE} and ti_tag.entity_id = t.id), t.notes, ${'Uncategorized'})`.as(
        'classification',
      ),
    ])
    .where('t.owner_userid', '=', parsed.userId);
  ...
  if (tagIds.length > 0 && tagNames.length > 0) {
    query = query.where(
      sql<boolean>`exists (select 1 from app.tag_assignments ti_filter join app.tags tg_filter on tg_filter.id = ti_filter.tag_id and tg_filter.owner_userid = ${parsed.userId} where ti_filter.entity_table = ${FINANCE_TRANSACTION_ENTITY_TYPE} and ti_filter.entity_id = t.id and (ti_filter.tag_id in (${sqlValueList(tagIds)}) or tg_filter.name in (${sqlValueList(tagNames)})))`,
    );
  } else if (tagIds.length > 0) {
    query = query.where(
      sql<boolean>`exists (select 1 from app.tag_assignments ti_filter join app.tags tg_filter on tg_filter.id = ti_filter.tag_id and tg_filter.owner_userid = ${parsed.userId} where ti_filter.entity_table = ${FINANCE_TRANSACTION_ENTITY_TYPE} and ti_filter.entity_id = t.id and ti_filter.tag_id in (${sqlValueList(tagIds)}))`,
    );
  } else if (tagNames.length > 0) {
    query = query.where(
      sql<boolean>`exists (select 1 from app.tag_assignments ti_filter join app.tags tg_filter on tg_filter.id = ti_filter.tag_id and tg_filter.owner_userid = ${parsed.userId} where ti_filter.entity_table = ${FINANCE_TRANSACTION_ENTITY_TYPE} and ti_filter.entity_id = t.id and tg_filter.name in (${sqlValueList(tagNames)}))`,
    );
  }
...
export async function getTagBreakdown(
  ...
  const result = await db
    ...
    .innerJoin('app.tags as tg', (join) =>
      join.onRef('tg.id', '=', 'ti.tag_id').on('tg.owner_userid', '=', _userId),
    )
    .select([
      sql<string>`tg.name`.as('tag'),
      sql<number>`coalesce(sum(abs(amount)), 0)`.as('total'),
    ])
    .where('t.owner_userid', '=', _userId)
