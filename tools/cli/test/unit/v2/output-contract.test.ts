...
function normalizeEnvelope(input: CommandSuccess<JsonValue>, hominemHome: string): NormalizedSuccessEnvelope {
  ...
  it('matches normalized json/ndjson envelopes for core commands', async () => {
    ...
    await withIsolatedPaths(hominemHome, async () => {
      for (const format of ['json', 'ndjson'] as const) {
        ...
        const doctorData = normalizedDoctor.data as { checks: Array<{ id: string; status: string; message: string }> }
        ...
        expect(doctorData.checks.some((check) => check.id === 'config.v2' && check.status === 'pass' && check.message === 'config version 2')).toBeTrue()
