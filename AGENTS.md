...
- `api`/`web`/`career`/`finance` run through the [portless](https://github.com/vercel-labs/portless) proxy under `pnpm dev` (each gets a stable `https://<name>.localhost:4200` URL instead of a fixed port, which is what lets the same app run from multiple worktrees at once without a port collision). A fresh worktree also needs its git-ignored `.env` files bootstrapped before anything will start. Both are one-time setup steps covered in the `hominem-development` skill — read it before telling the user a service "won't start" in a new worktree.
