...
- Agents may start local dev services (Expo/Metro, `pnpm dev`, the API, workers, databases, Docker containers, etc.) when needed to test or verify a change. `api`/`web`/`career`/`finance` run through the [portless](https://github.com/vercel-labs/portless) proxy under `pnpm dev` (each gets a stable `https://<name>.localhost:4200` URL instead of a fixed port, which is what lets the same app run from multiple worktrees at once without a port collision), and a fresh worktree needs its git-ignored `.env` files bootstrapped first. See the `hominem-development` skill for that setup and how to run services — read it before telling the user a service "won't start" in a new worktree.
